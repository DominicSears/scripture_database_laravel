<x-app-layout>
    <div class="flex flex-col w-full pb-8 space-y-6 pb-12 pt-8 px-6">
        <div class="bg-white w-full rounded-2xl shadow-xl h-fit flex flex-col space-y-2">
            ...
            <!-- Header Profile Info -->
            <div class="flex flex-row space-x-2 justify-between p-4 bg-white rounded-2xl h-fit">
                <!-- Profile section -->
                <div class="flex flex-row space-x-4">
                    @if (empty($user->profile_photo_path))
                        <div class="rounded-full h-52 w-52 -mt-14 ml-2 border-4 border-white shadow-xl bg-gray-700"></div>
                    @else
                        <img class="rounded-full h-52 w-52 -mt-14 ml-2 border-4 border-white shadow-xl"
                             src="{{ $user->profile_photo_url }}"
                             alt="{{ $user->username }}" />
                    @endif
                    <!-- Profile info and Accolades -->
                    <div class="flex flex-col space-y-4 justify-between">
                        <!-- Profile Info and Follow -->
                        <div class="flex flex-col space-y-2">
                            <div class="flex flex-col space-y-1">
                                <h2 class="text-3xl font-bold text-sky-900">{{ '@' . $user->username }}</h2>
                                <p class="text-md text-slate-500 font-bold">
                                    <span>{{ $user->faith_title }}</span>
                                </p>
                            </div>
                            <button class="w-fit py-2 px-6 bg-sky-300 hover:bg-sky-400 text-white font-bold rounded-lg">
                                <span>Follow</span>
                            </button>
                        </div>
                    </div>
                ...
                <!-- Stats section -->
                <div class="h-fit flex flex-row space-x-8">
                    <!-- Column 1 -->
        ...
        <!-- Filter Section -->
        <div class="flex flex-row space-x-2 px-2 py-2 bg-sky-100 rounded-xl w-fit shadow-xl">
            <button @class([
            'text-sky-300 rounded-lg px-4 py-2',
        ...
        <!-- Profile Section -->
        <div class="flex flex-col space-y-4 p-8 rounded-2xl shadow-2xl bg-white h-96"></div>
    </div>
</x-app-layout>
